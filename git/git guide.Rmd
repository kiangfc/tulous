...
> 工作流:working dir-add->index(stage)-commit->head
...
## 分支:branch->feature_x|master->merge
...
安装好 Git 后，打开命令行工具，进入工作文件夹(cd,mkdir)

进入：https://github.com/kiangfc/tulous，点击 Clone or download ，再点击 Use HTTPS，复制项目地址 ：
https://github.com/kiangfc/tulous.git 备用。

1. git clone:从git服务器拉取代码

>git clone https://github.com/kiangfc/tulous.git 

代码下载完成后在当前文件夹中会有一个 tulous 的目录，通过 cd tulous 命令进入目录。

2. git config:配置开发者用户名和邮箱

>git config user.name kiangfc

>git config user.email kiangfc@126.com

每次代码提交的时候都会生成一条提交记录，其中会包含当前配置的用户名和邮箱。

3. git branch:创建、重命名、查看、删除项目分支，通过 Git 做项目开发时，一般都是在开发分支中进行，开发完成后合并分支到主干。
...
12. .gitignore:设置哪些内容不需要推送到服务器，这是一个配置文件
...
通过此命令将打开交互式子命令系统，通过输入序列号或首字母可以选择相应的功能，具体的功能解释如下：
...
直接进入交互命令中最有用的 patch 模式。这是交互命令中最有用的模式，其操作方式和 update 类似，选择后 Git 会显示这些文件的当前内容与本地版本库中的差异，然后您可以自己决定是否添加这些修改到暂存区，在命令行 Stage deletion [y,n,q,a,d,/,?]? 后输入 y,n,q,a,d,/,? 其中一项选择操作方式，具体功能解释如下：
...
2. git cherry-pick:合并分支的一条或几条提交记录到当前分支末梢

>git cherry-pick 170a305

合并提交ID 170a305 到当前分支末梢。

3. git reset：将当前的分支重设（reset）到指定的 <commit> 或者 HEAD
...
4. git rebase：重新定义分支的版本库状态

>git rebase branch_name

合并分支，这跟 merge 很像，但还是有本质区别（~~缺示意图~~）。合并过程中可能需要先解决冲突，然后执行 git rebase --continue
...
5. git revert：撤销某次操作，此次操作之前和之后的 commit 和 history 都会保留，并且把这次撤销作为一次最新的提交
...
6. git diff：查看工作区、暂存区、本地版本库之间的文件差异，用一张图来解释（~~缺图~~）

>git diff --stat

通过 --stat 参数可以查看变更统计数据。

7. git reflog：reflog 可以查看所有分支的所有操作记录（包括commit和reset的操作、已经被删除的commit记录，跟 git log 的区别在于它不能查看已经删除了的commit记录

## 远程版本库连接

如果在GitHub项目初始化之前，文件已经存在于本地目录中，那可以在本地初始化本地版本库，再将本地版本库跟远程版本库连接起来。

1. git init：在本地目录内部会生成.git文件夹
...
1. git blame：查看文件每行代码块的历史信息

>git blame -L 1,10 demo.html

截取 demo.html 文件1-10行历史信息。

2. git bisect：二分查找历史记录，排查BUG
...
## 使用RStudio Project和Git版本控制

RStudio的版本控制功能需要与Project配合使用。启用步骤依赖于Project状态，目前主要包括以下四种情况：

### 工作目录已处于版本控制中

通过 "File | New Project"  →  "Existing Directory" 进入创建界面，基于已处于版本控制的工作目录所创建的 Project，可自动开启版本控制功能。

### 从版本控制存储库中克隆项目

通过 "File | New Project" → "Version Control" → "Git"/"Subversion " 进入创建界面，基于版本控制系统克隆项目所创建Project，可自动开启版本控制功能。

### 对已存在项目添加版本控制功能

通过 "Tools | Project Options" → "Git/SVN" → 将 "None" 更为 "Git " 。

### 创建新项目，并添加版本控制功能

通过菜单栏 "File | New Project" → "New Directory" → "New Project" → 勾选 "Create a git repository" 进入创建界面，并同时创建Git库。

## RStudio中的Git操作

版本控制中的RStudio Project界面会增加Git面板和下拉菜单，常用的add、diff、commit、push、pull、branch等命令可以通过点击操作按钮实现。

## 使用terminal

由于RStudio并未提供所有Git命令的交互式操作，部分命令需要在终端中执行，打开方式为：菜单栏 "Tools | Terminal" → "New Terminal"。

## 与GitHub连接（选用）

倘若想结合使用GitHub，需要完成下述步骤：

### 1） 在GitHub上创建一个库并复制url

### 2） 将RStudio Project与远程库连接
在RStudio Project 的 terminal 中执行下述命令，其中“GitHub-url”填写上一步所复制的url地址：

>$ git remote add origin <GitHub-url>

此时打开Project Options，远程库地址已经添加成功。
